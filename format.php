...
// Display the whole course as "qmulweeks" made of of modules
 ...
 *   1. Straight copy of qmulweeks/format.php
...
$strshowallqmulweeks = get_string('showallweeks');
...
if ($editing) {
    ...
    $strqmulweekshow = get_string('showweekfromothers');
...
if ($thissection->summary or $thissection->sequence or $PAGE->user_is_editing()) {
    ...
    echo $OUTPUT->heading(format_string($subcat->name), 2, 'schoolname');
    echo $OUTPUT->heading(format_string($course->fullname), 2, 'coursename');

    $coursecontext = get_context_instance(CONTEXT_COURSE, $course->id);
    if ($PAGE->user_is_editing() && has_capability('moodle/course:update', $coursecontext)) {
        echo '<p class="clearfix"><a title="' . get_string('editnewssettings', 'format_qmultopics') . '" ' .
    ...
    if ($newssettings = $DB->get_record('format_qmultopics_news', array('courseid' => $course->id))) {
        if ($newssettings->displaynews) {
            ...
            echo format_qmultopics_getimage($course, $newssettings->alttext);
            echo '<div class="static-text"><div class="static-padding">'.$newstext.'</div></div>';
...
while ($weekdate < $course->enddate) {
    ...
    if ($showsection) {
        ...
        if ($displaysection == $section) {
            echo '<a href="view.php?id=' . $course->id . '&amp;week=0#section-' . $section . '" title="' . $strshowallqmulweeks . '">' .
            '<img src="' . $OUTPUT->pix_url('i/all') . '" class="icon wkall" alt="' . $strshowallqmulweeks . '" /></a><br />';
        } else {
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', get_context_instance(CONTEXT_COURSE, $course->id))) {
            if ($thissection->visible) {        // Show the hide/show eye
                ...
                echo '<a href="view.php?id=' . $course->id . '&amp;show=' . $section . '&amp;sesskey=' . sesskey() . '#section-' . $section . '" title="' . $strqmulweekshow . '">' .
                '<img src="' . $OUTPUT->pix_url('i/show') . '" class="icon hide" alt="' . $strqmulweekshow . '" /></a><br />';
            }
